...
class SectionControllerTest extends FunctionalControllerTestCase
{
    ...
    private function checkStatusCodes($path)
    {
        ...
        $this->anonClient->request('GET', $this->u('/' . $this->path . $path, true));
        ...
        $this->adminClient->request('GET', $this->u('/' . $this->path . $path, true));
    ...
    {
        // test index response
        $crawler1 = $this->adminClient->request('GET', $this->u('/' . $this->path . '/', true));
        $this->assertEquals(200, $this->adminClient->getResponse()->getStatusCode());
        ...
        $crawler = $this->adminClient->click($crawler1->filter('a.card.fluid')->first()->link());
        $this->assertEquals(200, $this->adminClient->getResponse()->getStatusCode());
        ...
        $this->adminClient->click($crawler->filter('a.card.fluid')->first()->link());
        $this->assertEquals(200, $this->adminClient->getResponse()->getStatusCode());


        // click on last subsection
        $this->adminClient->click($crawler1->filter('a.card.fluid')->last()->link());
        $this->assertEquals(200, $this->adminClient->getResponse()->getStatusCode());
    }
}