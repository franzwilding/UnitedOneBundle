...
class TableControllerTest extends FunctionalControllerTestCase
{

    private $path = 'tablecontrollertest';
    ...
    public function testCRUDActions()
    {
        ...
        $this->assertGreaterThan(0, $crawler->filter('a:contains("Add")')->count());

        // Click on the create link
        $crawler = $this->adminClient->click($crawler->filter('a:contains("Add")')->first()->link());
        $this->assertEquals(200, $this->adminClient->getResponse()->getStatusCode());
        ...
        // Fill out form
        $this->adminClient->submit($form->form(), array('form[title]' => 'New Entity'));
        ...
        $this->assertGreaterThan(0, $crawler->filter('a:contains("Add")')->count());
        $this->assertGreaterThan(0, $crawler->filter('td:contains("New Entity")')->count());

        // Update the entity
        $crawler = $this->adminClient->click($crawler->filter('a:contains("Edit")')->first()->link());
        $this->assertEquals(200, $this->adminClient->getResponse()->getStatusCode());
        ...
        $this->assertGreaterThan(0, $form->filter('a:contains("Cancel")')->count());

        // Hit the cancel button
        $crawler = $this->adminClient->click($crawler->filter('a:contains("Cancel")')->first()->link());
        $this->assertEquals(200, $this->adminClient->getResponse()->getStatusCode());
        ...
        // Fill out form
        $this->adminClient->submit($form->form(), array('form[title]' => 'Updated Entity'));
        ...
        $this->assertGreaterThan(0, $crawler->filter('a:contains("Add")')->count());
        $this->assertGreaterThan(0, $crawler->filter('td:contains("Updated Entity")')->count());

        // Click on the Update Entity link to get to the delete button
        $crawler = $this->adminClient->click($crawler->filter('a:contains("Edit")')->first()->link());
        $this->assertEquals(200, $this->adminClient->getResponse()->getStatusCode());

        $this->assertGreaterThan(0, $crawler->filter('a:contains("Delete")')->count());
        $crawler = $this->adminClient->click($crawler->filter('a:contains("Delete")')->first()->link());

        // Now we should see an delete confirmation screen
        $this->assertGreaterThan(0, $crawler->filter('button:contains("Delete")')->count());
        $this->assertGreaterThan(0, $crawler->filter('a:contains("Cancel")')->count());
