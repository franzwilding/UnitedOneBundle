...
class CollectionControllerTest extends FunctionalControllerTestCase
{
    ...
    public function testCRUDActions()
    {
        ...
        $this->assertEquals(
          1,
          $crawler->filter(
            'a.positive.button:contains("Create the first collection")'
          )->count()
        );
        ...
        $this->adminClient->submit(
          $form->form(),
          array('form[title]' => 'First Collection Entity')
        );
        ...
        $this->assertEquals(
          1,
          $menu->filter('a.active.item:contains("First Collection Entity")')
            ->count()
        );
        $this->assertEquals(
          1,
          $menu->filter('a.item:contains("Add collection")')->count()
        );

        // And there should be the first collection item with edit button and it's child controllers
        $this->assertEquals(
          1,
          $crawler->filter('h2:contains("First Collection Entity")')->count()
        );
        $this->assertGreaterThan(
          0,
          $crawler->filter('a.button:contains("Edit")')->count()
        );

        // click on collection edit
        $crawler = $this->adminClient->click(
          $crawler->filter('a.button:contains("Edit")')->first()->link()
        );
        ...
        $this->adminClient->submit(
          $form->form(),
          array('form[title]' => 'Updated Collection Entity')
        );
        $this->assertEquals(
          302,
          $this->adminClient->getResponse()->getStatusCode()
        ...
        $this->assertEquals(
          1,
          $crawler->filter('h2:contains("Updated Collection Entity")')->count()
        );


        // click on add new collection
        $crawler = $this->adminClient->click(
          $crawler->filter('a.item:contains("Add collection")')->first()->link()
        );
        $form = $crawler->filter('form')->first();
        $this->adminClient->submit(
          $form->form(),
          array('form[title]' => '2nd Collection Entity')
        );
        $this->assertEquals(
          302,
          $this->adminClient->getResponse()->getStatusCode()
        ...
        $this->assertEquals(
          1,
          $menu->filter('a.active.item:contains("Updated Collection Entity")')
            ->count()
        );
        $this->assertEquals(
          1,
          $menu->filter('a.item:contains("2nd Collection Entity")')->count()
        );

        // navigate to the new collection
        $crawler = $this->adminClient->click(
          $crawler->filter('a.item:contains("2nd Collection Entity")')
            ->first()
            ->link()
        );
        $menu = $crawler->filter('div.ui.menu')->first();
        $this->assertEquals(
          1,
          $crawler->filter('h2:contains("2nd Collection Entity")')->count()
        );
        $this->assertEquals(
          1,
          $menu->filter('a.item:contains("Updated Collection Entity")')->count()
        );
        $this->assertEquals(
          1,
          $menu->filter('a.active.item:contains("2nd Collection Entity")')
            ->count()
        );
        ...
        $crawler = $this->adminClient->click(
          $table->filter('a.positive.button:contains("Add")')->first()->link()
        );
        $form = $crawler->filter('form')->first();
        $this->adminClient->submit(
          $form->form(),
          array('form[title]' => 'New Entity with table controller')
        );
        $this->assertEquals(
          302,
          $this->adminClient->getResponse()->getStatusCode()
