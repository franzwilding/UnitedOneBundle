...
class CardControllerTest extends FunctionalControllerTestCase
{
    ...
    public function testCRUDActions()
    {
        ...
        $this->assertGreaterThan(0, $crawler->filter('a:contains("Add")')->count());
        $this->assertGreaterThan(0, $crawler->filter('a:contains("New Entity")')->count());

        // Update the entity
        $crawler = $this->adminClient->click($crawler->filter('a:contains("New Entity")')->first()->link());
        $this->assertEquals(200, $this->adminClient->getResponse()->getStatusCode());
        ...
        $this->assertGreaterThan(0, $crawler->filter('a:contains("Add")')->count());
        $this->assertGreaterThan(0, $crawler->filter('a:contains("Updated Entity")')->count());

        // Click on the Update Entity link to get to the delete button
        $crawler = $this->adminClient->click($crawler->filter('a:contains("Updated Entity")')->first()->link());
        $this->assertEquals(200, $this->adminClient->getResponse()->getStatusCode());
